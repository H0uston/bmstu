...
class MainWindow(QMainWindow):
    ...
    def do_plan(self):
        ...
        try:
            ...
            for exp in self.plan:
                proc_int = scale_factor(exp[1], gen_int_min, gen_int_max)
                gen_int = scale_factor(exp[2], proc_int_min, proc_int_max)
                gen_dev = scale_factor(exp[3], proc_dev_min, proc_dev_max)

                proc = [Generator(rayleigh_by_intensity, (proc_int,))]
                gens = Generator(uniform_by_intensity, (gen_int, gen_dev))
                model = EventModel(proc, gens, total_apps)
    ...
    def set_equasions(self, b, accuracy=3):
        yl = str(round(b[0], accuracy)) + " + " + str(round(b[1], accuracy)) + "x1 + " + str(
            round(b[3], accuracy)) + "x2 + " + str(round(b[2], accuracy)) + "x3"
        yl = yl.replace("+ -", "- ")
