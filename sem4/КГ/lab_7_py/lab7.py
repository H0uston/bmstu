...
from PyQt5.QtGui import QPen, QColor, QImage, QPixmap, QPainter, QTransform, QPalette
...
class Window(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.segment_colour = Qt.black
        self.cutter_colour = Qt.red
        self.cut_colour = Qt.blue
        ...
        self.changeSegmentColourButton.clicked.connect(lambda: changeSegmentColour(self))
        self.changeCutterColourButton.clicked.connect(lambda: changeCutterColour(self))
        self.changeCutColourButton.clicked.connect(lambda: changeCutColour(self))

        palette = QPalette()
        palette.setColor(QPalette.Background, self.segment_colour)
        self.segmentFrame.setAutoFillBackground(True)
        self.segmentFrame.setPalette(palette)
        self.cutterFrame.setAutoFillBackground(True)
        palette.setColor(QPalette.Background, self.cutter_colour)
        self.cutterFrame.setPalette(palette)
        self.cutFrame.setAutoFillBackground(True)
        palette.setColor(QPalette.Background, self.cut_colour)
        self.cutFrame.setPalette(palette)
        ...
        self.line_pen = QPen(self.segment_colour)
        self.cutter_pen = QPen(self.cutter_colour)
        self.cut_pen = QPen(self.cut_colour)
    ...
    def mouseMoveEvent(self, event):
        ...
        if w.input_rect:
            if now is None:
                ...
                p = event.scenePos()
                self.addRect(now.x(), now.y(), abs(now.x() - p.x()), abs(now.y() - p.y()), w.cutter_pen)


def changeSegmentColour(win):
    colour = QtWidgets.QColorDialog.getColor(Qt.white, win, "Выберите цвет")
    if colour.isValid():
        palette = QPalette()
        palette.setColor(QPalette.Background, colour)
        win.segmentFrame.setPalette(palette)
        win.segment_colour = colour
        win.line_pen = QPen(win.segment_colour)


def changeCutterColour(win):
    colour = QtWidgets.QColorDialog.getColor(Qt.white, win, "Выберите цвет")
    if colour.isValid():
        palette = QPalette()
        palette.setColor(QPalette.Background, colour)
        win.cutterFrame.setPalette(palette)
        win.cutter_colour = colour
        win.cutter_pen = QPen(win.cutter_colour)


def changeCutColour(win):
    colour = QtWidgets.QColorDialog.getColor(Qt.white, win, "Выберите цвет")
    if colour.isValid():
        palette = QPalette()
        palette.setColor(QPalette.Background, colour)
        win.cutFrame.setPalette(palette)
        win.cut_colour = colour
        win.cut_pen = QPen(win.cut_colour)
...
def add_point(point):
    ...
    if w.input_bars:
        if w.point_now is None:
            ...
            global shiftFlag
            if shiftFlag:
                if math.fabs(w.point_now.x() - point.x()) < math.fabs(w.point_now.y() - point.y()):
                    line = [[w.point_now.x(), w.point_now.y()],
            ...
            w.scene.addLine(line[0][0], line[0][1], line[1][0], line[1][1], w.line_pen)
            w.point_now = None
...
def add_bars(win):
    ...
    if buf is None:
        ...
        w.table.setItem(i, 0, item_b)
        w.table.setItem(i, 1, item_e)
        win.scene.addLine(win.clip[0], win.clip[2] + t,  win.clip[0], win.clip[3] - t, win.line_pen)
        ...
        w.table.setItem(i, 0, item_b)
        w.table.setItem(i, 1, item_e)
        win.scene.addLine(win.clip[1], win.clip[3] - t,  win.clip[1], win.clip[2] + t, win.line_pen)
        ...
        w.table.setItem(i, 0, item_b)
        w.table.setItem(i, 1, item_e)
        win.scene.addLine(win.clip[0] + k, win.clip[2], win.clip[1] - k, win.clip[2], win.line_pen)
        ...
        w.table.setItem(i, 0, item_b)
        w.table.setItem(i, 1, item_e)
        win.scene.addLine(win.clip[0] + k, win.clip[3], win.clip[1] - k, win.clip[3], win.line_pen)
...
def cohen_sutherland(bar, rect, win):
    ...
    for i in range(4):
        ...
        if vis == 1:
            win.scene.addLine(bar[0][0], bar[0][1], bar[1][0], bar[1][1], win.cut_pen)
            return
        elif not vis:
            return
