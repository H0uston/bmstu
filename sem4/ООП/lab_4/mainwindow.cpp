...
void Widget::moveObject() {
    ...
    if (obj_name[0] == 'm') {
        ...
        Point scale(1, 1, 1);
        commands::TransformShapeCommand command(obj_name, moving, scale, rotateing);
...
void Widget::rotateObjectX() {
    ...
    if (obj_name[0] == 'c') {
        commands::LongitudinalCameraCommand command(obj_name, angle);
        _facade->execute_command(&command);
    } else {
        Point moving(0, 0, 0);
...
void Widget::rotateObjectY() {
    ...
    double angle = ui->doubleSpinBox_4->value();
    if (obj_name[0] == 'c') {
        commands::TransverseCameraCommand command(obj_name, angle);
        _facade->execute_command(&command);
    } else {
        Point moving(0, 0, 0);
...
void Widget::rotateObjectZ() {
    ...
    double angle = ui->doubleSpinBox_4->value();
    if (obj_name[0] == 'c') {
        commands::VerticalCameraCommand command(obj_name, angle);
        _facade->execute_command(&command);
    } else {
        Point moving(0, 0, 0);
...
void Widget::addShape() {
    ...
    commands::AddShapeCommand command(file_name, model_name);
    try {
        _facade->execute_command(&command);
        render();
...
void Widget::removeObject() {
    ...
    std::string obj_name = ui->comboBox->currentText().toStdString();
    if (obj_name[0] == 'm') {
        commands::RemoveShapeCommand command(obj_name);
        _facade->execute_command(&command);
    } else {
...
void Widget::render() {
    ...
    _pixmap.fill();
    auto drawer = new drawing::QtDrawer(&painter);
    auto command = new commands::DrawCommand(std::shared_ptr<drawing::BaseDrawer>(drawer));
    try {
        _facade->execute_command(command);
        _draw_label->update();
